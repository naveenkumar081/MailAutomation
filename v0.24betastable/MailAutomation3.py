   ...
   # print(count1)
    def  __init__(self):
      ...
      #  print(mainwindow.count1)
        ...
        time.sleep(3)
        ar = open('EntertheHTMLPart-I.txt','r')
        # ar=a.read()
        mainwindow.readar=ar.read()
        time.sleep(3)
...
class Sample(tk.Frame,mainwindow):
    ...
    def multithreading(self):
        ...
        for i in range(0, len(mainwindow.readlg)):
            ...
            msg1=mainwindow.readar
            complete=msg+msg1
            message=MIMEMultipart('alternative')
                #textmessage = MIMEText(, 'html')
            htmlmessage=MIMEText(complete,'html')
            message['From'] =mainwindow.readxs[0]
            message['To'] = mainwindow.readlg[i]
                #bk=',Feedback-ID:{df}:{fd}:01:01'.format(df=df,fd=i)
            message['Subject'] =actsub
            # print(message['from'])
                # msg=str(message)
                #message.attach(textmessage)
            message.attach(htmlmessage)#self.subject.config(text=str(sub))
            if i <= 10:
                try:
                    if self.internetcheck() == False:
                        ...
                        # asking server to start
                        print(mainwindow.readxs[0])
                        print(mainwindow.readys[i])
                        print(server.login(mainwindow.readxs[0], mainwindow.readys[0]))  # Sender Mail, Sender Mail Password
                        print(mainwindow.readxs[0])
                        print(mainwindow.readlg[i])    
                        # print(msg_full)
                        print(server.sendmail(mainwindow.readxs[0], [mainwindow.readlg[i]], message.as_string()))
                       # print(self.lg,"s")
                        self.start_thread(mainwindow.readxs[0],mainwindow.readlg[i])
...
class Actual(tk.Frame,mainwindow):
                ...
                if i <= 500:
                    try:
                        ...
                        #tkinter.messagebox.showwarning('Warning',"Network Failue,waiting for connections")
                            ...
                            #print("enetring the server")
                            server = smtplib.SMTP('smtp.gmail.com:587')  # setting up service provider name : port number
                            server.starttls() 
                            #print("server login") # asking server to start
                       ...
                       #print("servercount=",servercount)
                        ...
                        #print("current j value=",j)
                            print(server.sendmail(mainwindow.readxs[j], [mainwindow.readls[i]],message.as_string()))
                            d={mainwindow.readxs[j]:mainwindow.readls[i]}
                        ...
                        #print("servercount2=",servercount2)
                            ...
                            Actual.key=i         
                            #print("im at the end of server function")
                       # print(Actual.key)
                            # if k!=0:
                            #     if j<30:
                            #         j=1
                        ...
                        #self.text.insert(END,ment)
                            ...
                            file.write("Error: MailerID {} /n".format(i))
                            file.close()
                            #print("im having another problem")
                            # self.text.insert(END,"Date Time:{} \n".format(str(now)))
    ...
    def report(self):
        ...
        self.text.grid(row=3)
        print(reportval)
        for i in range(len(reportval)):
            self.text.insert(END,"Signin ID and Sent ID: = {} \n".format(reportval[i]))
        roots.geometry('%dx%d+%d+%d' % (self.k, self.s, self.z, self.u))
        roots.mainloop()                    
        #self.after(POLLING_TIME, self.queue_polling)            
...
#----- queue_polling2 of solve class-----#                
    ...
    def report(self):
        ...
        for i in range(len(reportval)):
            ...
            self.text.insert(END,"Signin ID SentId = {}  \n".format(reportval[i]))
        roots.geometry('%dx%d+%d+%d' % (self.k, self.s, self.z, self.u))
        roots.mainloop()
